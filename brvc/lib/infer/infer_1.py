...
def get_synth_model(device: torch.device) -> "SynthesizerTrnMsNSFsid":
    """
    Instantiate SynthesizerTrnMsNSFsid compatible with v2 + f0 RVC checkpoints
    using local ConfigV2 (keeps structure of your script).
    """
    from lib.modules.synthesizer_trn_ms import SynthesizerTrnMsNSFsid
    ...
    net_g = SynthesizerTrnMsNSFsid(
        ...
        lrelu_slope=0.1,
        # Important for v2: HuBERT embeddings are 768-d
        txt_channels=768,
    )
    net_g.to(device)
    net_g.eval()
...
def load_rvc_model(
    g_path: Path,
    device: torch.device,
    load_mode: Literal["rvc", "train"] = "rvc",
) -> Tuple[torch.nn.Module, int, str]:
    """
    Load an RVC v2 model with f0 (closer to RVC logic).
    - Reject v1 or no-f0 checkpoints for 'rvc' mode.
    - Returns (net_g, target_sr, version)
    """
    logger.info(f"Loading checkpoint: {g_path}")
    cpt = torch.load(g_path, map_location="cpu")
    
    version = cpt.get("version", "v1")
    if load_mode == "rvc":
        if version != "v2":
            raise ValueError(f"RVC v1 models are not supported. Got version '{version}'.")
        if_f0 = int(cpt.get("f0", 1))
        if if_f0 != 1:
            raise ValueError("This loader only supports v2 models WITH f0 (no 'nono').")
        weight_key = "weight"
    elif load_mode == "train":
        weight_key = "model"
    else:
        raise ValueError(f"Invalid load_mode: {load_mode}. Use 'rvc' or 'train'.")
