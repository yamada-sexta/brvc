...
def compare_arrays(arr1: np.ndarray, arr2: np.ndarray) -> Tuple[str, Optional[float]]:
    ...
    print(
        f"--- Comparing Array 1 (Length: {len(arr1)}) and Array 2 (Length: {len(arr2)}) ---"
    )
...
load_and_compare(
    f"{brvc}debug_final_output_audio.npy", f"{rvc}debug_final_output_audio.npy"
)
...
load_wav_and_compare(
    f"{brvc}debug_filtered_audio.wav", f"{rvc}debug_filtered_audio.wav"
)
