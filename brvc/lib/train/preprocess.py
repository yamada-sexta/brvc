...
def normalize_audio(
    ...
    # Guard against empty arrays to avoid numba/NumPy reduction errors.
    # Also guard against zero max to avoid divide-by-zero.
    if audio.size == 0:
        return None
    ...
    if tmp_max == 0.0:
        return None
    if tmp_max > 2.5:
        # Extremely large amplitude; skip this segment.
        return None
    ...
    try:
        ...
        for sliced_audio in slicer.slice(audio):
            ...
            while True:
                ...
                gt_file = gt_wavs_dir / basename
                wav16_file = wavs16k_dir / basename
...
def preprocess_dataset(
    ...
    if isinstance(dataset, Path) and not dataset.exists():
        raise FileNotFoundError(f"Audio collection path {dataset} does not exist.")
    ...
    if recursive:
        ...
        files = [p for p in dataset.iterdir() if p.suffix.lower() in audio_exts]
